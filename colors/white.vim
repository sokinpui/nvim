...
call s:h("Todo", { "fg": s:red, "cterm": "bold,italic" }) " anything that needs extra attention; mostly the keywords TODO FIXME and XXX
...
call s:h("StatusLine", { "fg": s:cyan, "bg": s:cursor_grey }) " status line of current window
call s:h("StatusLineNC", { "fg": s:comment_grey, "bg": s:black }) " status lines of not-current windows Note: if this is equal to "StatusLine" Vim will use "^^^" in the status line of the current window.
call s:h("StatusLineTerm", { "fg": s:cyan, "bg": s:cursor_grey }) " status line of current :terminal window
